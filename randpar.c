...
static int strm=2;         /* index of current stream */
...
double uniform(double a,double b)
    { /* 'uniform' returns a psuedo-random variate from a uniform     */
      ...
      if (a>b)    
         erro("uniform Argument Error: a > b");
      return(a+(b-a)*ranf());
    }
...
double randompar(int i,int n)
    { /* 'random' returns an integer equiprobably selected from the   */
      ...
      if (i>n)       erro("random Argument Error: i > n");
      n-=i; n=(n+1.0)*ranf();
      return(i+n);
    }
...
double erlang(double x,double s)
    { /* 'erlang' returns a psuedo-random variate from an erlang      */
      ...
      int i,k; double z;
      if (s>x)       erro("erlang Argument Error: s > x");
      z=x/s; k=(int)z*z;
...
double hyperx(double x,double s)
    { /* 'hyperx' returns a psuedo-random variate from Morse's two-   */
      ...
      /* deviation s, s>x.  */
      double cv,z,p;
      if (s<=x)       erro("hyperx Argument Error: s not > x");
